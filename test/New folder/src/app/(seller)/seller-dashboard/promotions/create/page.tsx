...
// --- SỬA DÒNG IMPORT NÀY ---
// Dùng "as" để đổi tên CrossSellSelector thành ProductSelector cho gọn
...
export default function CreateVoucherPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Convert các field số về dạng number để tránh lỗi string từ input
      const payload = {
        ...formData,
    ...
    <div className="p-6 max-w-3xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 rounded-lg shadow">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
           ...
           {/* Chỉ hiện maxDiscount nếu là Percentage */}
           {/* (Nếu bạn chưa implement logic check loại giảm giá ở state thì cứ hiện luôn cũng được) */}
           <div className="space-y-2">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <label className={`border p-4 rounded cursor-pointer flex flex-col items-center gap-2 transition
              ${formData.scope === 'SHOP' ? 'border-blue-500 bg-blue-50 ring-1 ring-blue-500' : 'hover:bg-gray-50'}`}>
            ...
            <label className={`border p-4 rounded cursor-pointer flex flex-col items-center gap-2 transition
              ${formData.scope === 'PRODUCT' ? 'border-purple-500 bg-purple-50 ring-1 ring-purple-500' : 'hover:bg-gray-50'}`}>
            ...
            <label className={`border p-4 rounded cursor-pointer flex flex-col items-center gap-2 transition
              ${formData.scope === 'CATEGORY' ? 'border-green-500 bg-green-50 ring-1 ring-green-500' : 'hover:bg-gray-50'}`}>
