...
const CategoryTreeManager = () => {
    ...
    const customCollisionDetection: CollisionDetection = useCallback((args) => {
        // 1. Lấy danh sách va chạm mặc định
        const pointerCollisions = pointerWithin(args);
        if (pointerCollisions.length === 0) return [];

        const activeId = args.active.id as string;

        // 2. Tìm ID cha của Node đang được kéo
        const activeParent = findParent(tree, activeId);
        const activeParentId = activeParent ? activeParent.id : 'root';

        // 3. Lọc: Chỉ giữ lại các va chạm HỢP LỆ (cùng chung 1 cha)
        const validCollisions = pointerCollisions.filter(collision => {
            const collisionId = collision.id as string;

            // Cho phép va chạm với chính nó hoặc với khung chứa (SortableContext) của nó
            if (collisionId === activeId || collisionId === activeParentId) return true;

            const collisionParent = findParent(tree, collisionId);
            const collisionParentId = collisionParent ? collisionParent.id : 'root';

            // Chỉ return true nếu bằng cấp (cùng parentId)
            return collisionParentId === activeParentId;
        });
