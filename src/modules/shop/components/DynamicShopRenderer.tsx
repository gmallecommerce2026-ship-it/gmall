...
const BannerSection = ({ config }: { config: any }) => {
  // Thay đổi từ config.images sang config.src hoặc đồng bộ với cấu trúc BE trả về
  const rawImage = config?.images || config?.src;
  if (!rawImage || rawImage.length === 0) return null;
