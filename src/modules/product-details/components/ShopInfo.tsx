  ...
  return (
    <div className="bg-white rounded-2xl p-6 flex flex-col md:flex-row items-center gap-6 shadow-[0px_4px_20px_rgba(0,0,0,0.05)] border border-gray-100">
      ...
      <div className="flex items-center gap-4 flex-shrink-0 w-full md:w-auto border-b md:border-b-0 pb-4 md:pb-0 md:border-r border-gray-100 md:pr-8">
        ...
        {/* [UPDATE] Bọc Avatar trong Link để click chuyển hướng */}
        <Link href={`/shop/${shop.id}`} className="relative cursor-pointer group">
          <img
            src={imgSrc || `https://ui-avatars.com/api/?name=${shop.name}`} 
            alt={shop.name}
            className="w-20 h-20 md:w-24 md:h-24 rounded-full border border-gray-100 object-cover group-hover:opacity-90 transition-opacity"
        ...
        <div className="flex flex-col gap-2">
          <div>
            <Link href={`/shop/${shop.id}`} className="text-lg font-bold text-gray-900 flex items-center gap-2 hover:text-orange-600 transition-colors cursor-pointer">
              {shop.name} 
              {shop.isOfficial && (
          ...
          <div className="flex gap-2 mt-1">
            ...
            {/* [FIX] Bỏ Link bọc ngoài Button, dùng onClick */}
            <Button
