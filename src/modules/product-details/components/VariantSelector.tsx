...
interface VariantSelectorProps {
  ...
  // [NEW] Callback khi hover vào option
  onHoverOption?: (image: string | null) => void;
}
    ...
    <div className="flex flex-col gap-3 mb-4">
      ...
      <div className="flex flex-wrap gap-3">
        {tier.options.map((optionName, idx) => {
          ...
          const imageUrl = hasImage ? tier.images![idx] : null;

          return (
            <button
              key={`${tier.name}-${idx}`}
              onClick={() => onSelect(idx)}
              // [NEW] Thêm sự kiện hover để đổi ảnh
              onMouseEnter={() => {
                if (hasImage && onHoverOption && imageUrl) {
                  onHoverOption(imageUrl);
                }
              }}
              // Khi rời chuột có thể set null nếu muốn quay về ảnh cũ, 
              // nhưng Shopee thường giữ nguyên ảnh vừa hover cho trải nghiệm mượt.
              ...
              className={clsx(
                ...
                hasImage 
                  ? [
                      "p-1 h-16 min-w-[4rem] flex flex-col items-center justify-center gap-1",
                      isSelected 
                        ? "border-brand-orange bg-orange-50 ring-1 ring-brand-orange" 
                        : "border-gray-200 bg-white hover:border-gray-400"
                    ]
                  
                // --- TRƯỜNG HỢP 2: CHỈ CÓ CHỮ ---
                  : [
                      "px-4 py-2 min-w-[3rem] text-sm font-medium",
                      isSelected
