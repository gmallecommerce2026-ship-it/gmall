...
export const BlogCard: React.FC<BlogCardProps> = ({ 
  ...
  const dateStr = post.createdAt ? format(new Date(post.createdAt), 'dd/MM/yyyy', { locale: vi }) : '';
  ...
  if (variant === 'overlay') {
    return (
      <Link href={`/blog/${post.slug}`} className={cn("group relative block overflow-hidden w-full h-full", cardRounded, className)}>
        <Image src={post.thumbnail} alt={post.title} fill className="object-cover transition-transform duration-500 group-hover:scale-105" />
  ...
  if (variant === 'hero') {
    return (
      <Link href={`/blog/${post.slug}`} className={cn("group relative block h-full overflow-hidden w-full", className)}>
        <Image src={post.thumbnail} alt={post.title} fill className="object-cover transition-transform duration-700 group-hover:scale-105" />
    ...
    <div className={cn("group flex flex-col h-full bg-white", className)}>
      ...
      <div className="flex flex-col flex-grow">
        ...
        <div className="mt-2 flex items-center text-[11px] text-gray-500 gap-2 font-medium">
           <span className="uppercase tracking-wide">{post.author?.name}</span>
           <span className="text-gray-300">|</span>
           <span>{dateStr}</span>
        </div>
        {post.excerpt && (
           <p className="mt-2 line-clamp-2 text-xs text-gray-500 font-normal leading-relaxed">{post.excerpt}</p>
        )}
      </div>
    </div>
  );
