...
export const BlogHeader = ({ categories, onSearch, searchValue, onCategorySelect }: any) => {
  // Biến đổi flat list thành tree cho menu và lọc bỏ các danh mục cần ẩn
  const categoryTree = useMemo(() => {
    const tree = buildCategoryTree(categories || []);

    // Định nghĩa danh sách các slug hoặc từ khóa tên cần ẩn
    // (Bạn có thể điều chỉnh chính xác theo slug trong database của bạn)
    const excludedSlugs = ['ve-chung-toi', 'chinh-sach', 'ho-tro-khach-hang'];
    const excludedKeywords = ['về chúng tôi', 'chính sách', 'hỗ trợ khách hàng'];

    return tree.filter((cat: Category) => {
      const slugLower = (cat.slug || '').toLowerCase();
      const nameLower = (cat.name || '').toLowerCase();

      // Kiểm tra nếu trùng slug hoặc tên có chứa từ khóa loại trừ
      const isExcluded =
        excludedSlugs.includes(slugLower) ||
        excludedKeywords.some(keyword => nameLower.includes(keyword));

      return !isExcluded; // Chỉ giữ lại các danh mục không nằm trong danh sách ẩn
    });
  }, [categories]);
