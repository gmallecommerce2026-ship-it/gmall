...
import { Trash2, Edit, GripVertical, Plus, Save, X, LayoutTemplate, Zap, List } from 'lucide-react';

// --- TYPE DEFINITIONS ---
type SectionType = 'CATEGORY' | 'CATEGORY_TWO_ROW' | 'FLASH_SALE';
    ...
    <div className="bg-gray-50 h-full p-3 overflow-y-auto no-scrollbar font-sans text-xs">
       ...
       <div className="bg-white rounded shadow-sm border border-gray-100 overflow-hidden">
          ...
          {type === 'CATEGORY_TWO_ROW' ? (
            ...
            /* CASE 2: NORMAL / FLASH SALE */
             <div className="p-3">
                <div className="flex justify-between items-end mb-2">
                   <div className="flex items-center gap-1">
                      {type === 'FLASH_SALE' && <span className="text-red-500 font-bold text-xs">⚡</span>}
       ...
       <div className="flex-1 flex flex-col h-full overflow-hidden">
          <div className="flex justify-between items-center mb-6 shrink-0">
            <div>
              ...
              <p className="text-gray-500 text-sm">Kéo thả để sắp xếp thứ tự hiển thị</p>
            </div>
            <button 
                          ...
                          >
                            ...
                            <div className="flex-1">
                              <div className="flex items-center gap-3">
                                {section.type === 'CATEGORY_TWO_ROW' && <LayoutTemplate size={18} className="text-purple-500" />}
                                {section.type === 'FLASH_SALE' && <Zap size={18} className="text-red-500" />}
                                {section.type === 'CATEGORY' && <List size={18} className="text-blue-500" />}
...
function EditorModal({ initialData, onClose, onSave }: any) {
  ...
  const [config, setConfig] = useState(initialData?.config || {
    ...
    titleSize: 'text-lg',
    showPopup: false,
    left: { title: 'Bán chạy', emoji: '🔥', filters: [], headerColor: 'text-orange-600' },
    right: { title: 'Mới nhất', emoji: '🆕', filters: [], headerColor: 'text-blue-600' }
  });
  ...
  const handleSubmit = () => {
    // Nếu là Two Row, title chính chỉ để quản lý, đặt tự động nếu trống
    ...
    onSave({ 
      ...
      type, 
      config, 
      categoryId: type === 'CATEGORY' ? categoryId : '', // Chỉ gửi categoryId nếu đúng type
      voucherId: type === 'FLASH_SALE' ? voucherId : ''
    });
  };

  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-[1000px] h-[85vh] flex overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200">
        ...
        <div className="flex-1 flex flex-col h-full border-r border-gray-100">
              ...
              <section>
                 ...
                 <div className="grid grid-cols-3 gap-4">
                    <TypeCard 
                      ...
                      icon={<List size={24} />} 
                      title="Danh mục đơn" 
                      desc="Hiển thị 1 danh mục sản phẩm (ngang/dọc)" 
                    />
                    <TypeCard 
                      active={type === 'CATEGORY_TWO_ROW'} 
                      onClick={() => setType('CATEGORY_TWO_ROW')} 
                      icon={<LayoutTemplate size={24} />} 
                      title="Chia đôi (2 Cột)" 
                      desc="Hiển thị 2 danh mục cạnh nhau (Split View)" 
                    />
                    <TypeCard 
                      active={type === 'FLASH_SALE'} 
                      onClick={() => setType('FLASH_SALE')} 
                      icon={<Zap size={24} />} 
                      title="Flash Sale" 
                      desc="Đếm ngược & Sản phẩm từ Voucher" 
                    />
                 </div>
              </section>
                 ...
                 {type === 'CATEGORY_TWO_ROW' && (
                    <div className="space-y-6">
                       <div className="bg-blue-50/50 p-4 rounded-xl border border-blue-100">
                          <label className="text-xs font-bold text-blue-600 uppercase mb-3 block">Cột Bên Trái (Left)</label>
                          <div className="grid grid-cols-12 gap-3">
                             <div className="col-span-2">
                                <label className="text-[10px] text-gray-500 mb-1 block">Emoji</label>
                                <input className="w-full border rounded p-2 text-center" value={config.left.emoji} onChange={e => handleDeepConfig('left', 'emoji', e.target.value)} />
                             </div>
                             <div className="col-span-10">
                                <label className="text-[10px] text-gray-500 mb-1 block">Tiêu đề hiển thị</label>
                                <input className="w-full border rounded p-2 font-medium" value={config.left.title} onChange={e => handleDeepConfig('left', 'title', e.target.value)} placeholder="VD: Bán chạy nhất" />
                             </div>
                             <div className="col-span-12">
                                <label className="text-[10px] text-gray-500 mb-1 block">Filter Tags (Cách nhau dấu phẩy)</label>
                                <input className="w-full border rounded p-2 text-sm" placeholder="Áo thun, Quần Jean..." 
                                  value={Array.isArray(config.left.filters) ? config.left.filters.join(',') : config.left.filters}
                                  onChange={e => handleDeepConfig('left', 'filters', e.target.value.split(','))}
                                />
                             </div>
                          </div>
                       </div>
                       ...
                       <div className="bg-orange-50/50 p-4 rounded-xl border border-orange-100">
                          <label className="text-xs font-bold text-orange-600 uppercase mb-3 block">Cột Bên Phải (Right)</label>
                          <div className="grid grid-cols-12 gap-3">
                             <div className="col-span-2">
                                <label className="text-[10px] text-gray-500 mb-1 block">Emoji</label>
                                <input className="w-full border rounded p-2 text-center" value={config.right.emoji} onChange={e => handleDeepConfig('right', 'emoji', e.target.value)} />
                             </div>
                             <div className="col-span-10">
                                <label className="text-[10px] text-gray-500 mb-1 block">Tiêu đề hiển thị</label>
                                <input className="w-full border rounded p-2 font-medium" value={config.right.title} onChange={e => handleDeepConfig('right', 'title', e.target.value)} placeholder="VD: Sản phẩm mới" />
                             </div>
                             <div className="col-span-12">
                                <label className="text-[10px] text-gray-500 mb-1 block">Filter Tags</label>
                                <input className="w-full border rounded p-2 text-sm" placeholder="Tech, Decor..." 
                                   value={Array.isArray(config.right.filters) ? config.right.filters.join(',') : config.right.filters}
                                   onChange={e => handleDeepConfig('right', 'filters', e.target.value.split(','))}
                                />
                             </div>
                          </div>
                       </div>
                       ...
                       <div className="bg-yellow-50 p-3 rounded border border-yellow-200 text-xs text-yellow-800">
                          ⚠️ Lưu ý: Với khối 2 cột, sản phẩm sẽ được lấy tự động dựa trên tiêu đề & filter (Smart Query).
                       </div>
                    </div>
                 )}

                 {/* --- CASE B: SINGLE CATEGORY / FLASH SALE --- */}
                 {type !== 'CATEGORY_TWO_ROW' && (
                    <div className="space-y-4">
                       <label className="text-xs font-bold text-gray-500 uppercase block tracking-wide">2. Nội Dung & Nguồn Dữ Liệu</label>
                       
                       <div className="grid grid-cols-4 gap-4">
                          <div className="col-span-3">
                             <label className="block text-sm font-medium mb-1">Tiêu đề hiển thị</label>
                             <input className="w-full border border-gray-300 p-2.5 rounded-lg focus:ring-2 focus:ring-brand-orange/20 outline-none" 
                               value={title} onChange={e => setTitle(e.target.value)} placeholder="Nhập tiêu đề khối..." 
                             />
                          </div>
                          <div>
                             <label className="block text-sm font-medium mb-1">Emoji</label>
                             <input className="w-full border border-gray-300 p-2.5 rounded-lg text-center" 
                               value={config.emoji || ''} onChange={e => handleChangeConfig('emoji', e.target.value)} placeholder="🔥" 
                             />
                          </div>
                       </div>
                       ...
                       <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                          {type === 'CATEGORY' ? (
                             <div>
                                <label className="block text-sm font-medium mb-1 text-blue-600">Nguồn Danh Mục (Category ID)</label>
                                <input className="w-full border p-2 rounded bg-white" placeholder="Nhập ID danh mục..." value={categoryId} onChange={e => setCategoryId(e.target.value)} />
                                <p className="text-[10px] text-gray-500 mt-1">Hệ thống sẽ lấy sản phẩm từ danh mục này.</p>
                             </div>
                          ) : (
                             <div>
                                <label className="block text-sm font-medium mb-1 text-red-600">Nguồn Voucher (Voucher ID)</label>
                                <input className="w-full border p-2 rounded bg-white" placeholder="Nhập ID Voucher..." value={voucherId} onChange={e => setVoucherId(e.target.value)} />
                                <p className="text-[10px] text-gray-500 mt-1">Sản phẩm áp dụng Voucher này sẽ hiển thị.</p>
                             </div>
                          )}
                       </div>
           ...
           <div className="p-6 border-t border-gray-100 bg-gray-50 flex justify-end gap-3 rounded-bl-2xl">
