...
interface Tier {
    name: string;
    options: string[];
    images: string[]; // [NEW] Thêm mảng chứa ảnh cho từng option
}
interface SkuRow {
    key: string;
    indices: number[];
...
// [NEW] Danh sách 14 thuộc tính mở rộng chuẩn SEO
...
const MediaItem = ({ 
    ...
    <div className={classNames(
        ...
        {/* Content Placeholder or Real Media */}
        ...
        {type === 'video' && !isCover && typeof index === 'number' && (
            <div className="absolute top-2 left-2 bg-black/60 backdrop-blur-sm text-white text-[10px] font-medium px-1.5 py-0.5 rounded">
                Video {index + 1}
            </div>
        )}
...
const SectionCard = ({ children, title, id, className }: { children: React.ReactNode, title?: string, id?: string, className?: string }) => (
  <div id={id} className={classNames("bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden scroll-mt-28", className)}>
    {title && (
...
// Updated InputField to accept value & onChange
...
const TextAreaField = ({ placeholder, value, onChange }: any) => (
    <textarea 
        className="w-full min-h-[120px] p-3 border border-gray-300 rounded-lg outline-none text-sm text-gray-900 placeholder:text-gray-400 focus:border-orange-500 focus:ring-4 focus:ring-orange-500/10 transition-all duration-200 resize-y"
...
const AddProductPage = () => {
  ...
  const [isShowMoreAttributes, setIsShowMoreAttributes] = useState(false); // [NEW] Toggle mở rộng
  const [extraAttributes, setExtraAttributes] = useState<Record<string, string>>({}); // [NEW] State lưu 14 thuộc tính
  ...
  // Sales (Variants) - Updated with images
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, type: 'image' | 'video' | 'sizeChart') => {
      if (e.target.files && e.target.files[0]) {
          ...
          try {
              setIsUploading(true);
              const realUrl = await uploadFileToR2(file);
              if (type === 'image') setImages(prev => [...prev, realUrl]);
              else if (type === 'video') setVideos(prev => [...prev, realUrl]);
              else if (type === 'sizeChart') setSizeChartImage(realUrl);
          } catch (error) {
  ...
  // [UPDATED] Add Option with Image Placeholder
  const addOptionToTier = (idx: number, val: string) => {
      if (!val.trim()) return;
      const n = [...tiers];
      if (!n[idx].options.includes(val)) { 
          n[idx].options.push(val); 
          n[idx].images.push(''); // Push empty string for image
          setTiers(n); 
      }
  };
  
  // [UPDATED] Remove Option & Image
  ...
  const handleSubmit = async () => {
      try {
          ...
          // [DEBUG 1] Kiểm tra State thô trước khi xử lý
          console.log("--- BẮT ĐẦU DEBUG TẠO SẢN PHẨM ---");
          console.log("1. Raw Tiers State:", tiers);
          console.log("2. Raw SkuRows State:", skuRows);
          ...
          // Logic lọc tiers: Chỉ lấy tier có tên VÀ có options
          const validTiers = tiers.filter(t => t.name && t.options.length > 0);
          console.log("3. Valid Tiers (Sau khi lọc):", validTiers);

          if (tiers.length > 0 && validTiers.length === 0) {
              console.warn("CẢNH BÁO: Bạn có nhập phân loại nhưng bị lọc hết vì thiếu Tên nhóm hoặc Option!");
              alert("Vui lòng nhập đầy đủ Tên nhóm phân loại (VD: Màu sắc) và các tùy chọn.");
              setIsLoading(false);
              return;
          }

          // Logic map variations
          ...
          console.log("4. Mapped Variations:", validVariations);
          ...
          const payload = {
              ...
              // Sử dụng biến đã log để gửi đi
              tiers: validTiers,
              variations: validVariations,
              crossSellIds,
          };
          
          console.log("5. FINAL PAYLOAD GỬI API:", payload);

          // Gọi API
          const response = await api.post('/seller/products', payload);
          
          console.log("6. API RESPONSE:", response); // Xem API trả về gì
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="hidden xl:block col-span-3 sticky top-24 z-10">
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="bg-gradient-to-r from-orange-500 to-orange-600 px-4 py-3.5 flex items-center gap-2">
               <div className="p-1 bg-white/20 rounded-full"><CheckCircle2 size={16} className="text-white"/></div> 
               <span className="text-white font-semibold text-sm">Chất lượng sản phẩm</span>
            </div>
            <div className="p-2">
              {SUGGESTIONS.map((item) => (
                <div key={item.id} className="flex items-start gap-3 p-3 hover:bg-gray-50 rounded-lg transition-colors">
                    <div className={classNames("mt-0.5", item.done ? "text-green-500" : "text-gray-300")}>
                        {item.done ? <CheckCircle2 size={18} /> : <div className="w-[18px] h-[18px] rounded-full border-2 border-current"></div>}
                    </div>
                    <span className={classNames("text-sm font-medium leading-snug", item.done ? "text-gray-500 line-through decoration-gray-300" : "text-gray-700")}>
                        {item.label}
                    </span>
                </div>
              ))}
        ...
        <div className="col-span-1 lg:col-span-8 xl:col-span-6 flex flex-col gap-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-1.5 flex items-center overflow-x-auto no-scrollbar sticky top-[70px] z-30">
              {TABS.map((tab) => (
                  <button 
                      ...
                      className={classNames(
                          "flex-1 px-4 py-2.5 text-sm font-medium whitespace-nowrap rounded-lg transition-all duration-200",
                          activeTab === tab.id
          ...
          {/* Section: Basic Info */}
          <SectionCard title="Thông tin cơ bản" id="basic">
              {/* ... (Giữ nguyên phần Ảnh bìa, Ảnh sản phẩm, Video, Tên, Ngành hàng, GTIN) ... */}
              <div className="mb-8">
              ...
              <div className="mb-8">
                ...
                <div className={classNames("grid gap-4 transition-all", imageRatio === '1:1' ? "grid-cols-4 md:grid-cols-5 lg:grid-cols-6" : "grid-cols-3 md:grid-cols-4 lg:grid-cols-5")}>
                    ...
                    {images.length < 9 && (<label><UploadBox label={`Thêm ảnh (${images.length}/9)`} ratio={imageRatio} /><input type="file" hidden accept="image/*" onChange={(e) => handleFileUpload(e, 'image')} /></label>)}
                </div>
              </div>
              ...
              <div className="mb-6">
                  ...
                  <div className="grid grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-4 mb-4">
                      {videos.map((vid, idx) => (<MediaItem key={idx} type="video" index={idx} ratio="1:1" url={vid} onRemove={() => setVideos(videos.filter((_, i) => i !== idx))} />))}
                      {videos.length < 5 && (<label><UploadBox label="Thêm Video" icon={<Video size={24}/>} ratio="1:1" /><input type="file" hidden accept="video/*" onChange={(e) => handleFileUpload(e, 'video')} /></label>)}
                  </div>
              </div>
              ...
              <div className="space-y-6">
                  ...
                  <div><FormLabel required>Ngành hàng</FormLabel><SelectField value={categoryId} onChange={(val: string) => setCategoryId(val)} options={categories} placeholder="Chọn ngành hàng phù hợp" /></div>
          ...
          {/* Section: Detailed Info - [UPDATED with More Attributes] */}
          <SectionCard title="Thông tin chi tiết" id="details">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                  ...
                  <div><FormLabel required>Xuất xứ</FormLabel><SelectField value={origin} onChange={setOrigin} options={['Việt Nam', 'Trung Quốc', 'Hàn Quốc']} placeholder="Chọn xuất xứ" /></div>
                  <div><FormLabel>Chất liệu</FormLabel><InputField value={material} onChange={(e: any) => setMaterial(e.target.value)} /></div>
                  <div><FormLabel>Kiểu dáng</FormLabel><InputField value={style} onChange={(e: any) => setStyle(e.target.value)} /></div>
                  
                  {/* [NEW] Expanded Attributes Area */}
                  {isShowMoreAttributes && EXTENDED_ATTRIBUTES.map((attr) => (
                      <div key={attr.key} className="animate-in fade-in slide-in-from-top-2 duration-300">
                          ...
                          <InputField 
                              ...
                              value={extraAttributes[attr.key] || ''}
                              onChange={(e: any) => setExtraAttributes(prev => ({ ...prev, [attr.key]: e.target.value }))}
                          />
                      </div>
                  ))}
                  ...
                  <div className="col-span-1 md:col-span-2">
                     <button 
                        onClick={() => setIsShowMoreAttributes(!isShowMoreAttributes)}
                        className="flex items-center gap-2 text-orange-600 text-sm font-medium hover:bg-orange-50 px-3 py-2 rounded-lg transition-colors w-full justify-center border border-dashed border-orange-200"
                     >
          ...
          <SectionCard title="Mô tả sản phẩm" id="desc">
              <TextAreaField value={desc} onChange={(e: any) => setDesc(e.target.value)} placeholder="Mô tả chi tiết về tính năng..." />
          </SectionCard>

          {/* Section: Sales Info - [UPDATED with Image Upload] */}
          <SectionCard title="Thông tin bán hàng" id="sales">
              <div className="mb-8">
                  ...
                  <div className="bg-gray-50 p-5 rounded-xl border border-gray-200 space-y-4">
                             ...
                             <div className="space-y-3">
                                            ...
                                            <div className="shrink-0 w-10 h-10">
                                                {tier.images[oIdx] ? (
                                                    <div className="w-full h-full rounded border border-gray-200 relative group overflow-hidden">
                                                        ...
                                                        <button 
                                                            onClick={() => removeTierImage(tIdx, oIdx)}
                                ...
                                <div className="flex items-center gap-3">
                                    <div className="flex-1 max-w-[200px]">
                                        <input 
                                            className="w-full text-sm border border-dashed border-orange-300 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-orange-100 placeholder:text-orange-400 text-orange-700 bg-orange-50/30"
                                            placeholder="+ Thêm phân loại (Enter)"
                                            onKeyDown={(e) => { if(e.key==='Enter') { addOptionToTier(tIdx, e.currentTarget.value); e.currentTarget.value=''; } }} 
                                        />
                                    </div>
                                </div>
                             </div>
                      ...
                      {tiers.length < 2 && (
                          <button onClick={addTier} className="flex items-center gap-2 text-orange-600 border border-orange-200 bg-white rounded-lg px-4 py-2 text-sm font-medium hover:bg-orange-50 transition-colors w-full justify-center border-dashed">
                              <Plus size={16} /> Thêm nhóm phân loại {tiers.length + 1}
                          </button>
                      )}
                  </div>
              </div>

              {/* SKU TABLE - Giữ nguyên logic cũ */}
              <div className="mb-8">
                        ...
                        <div className="border border-gray-200 rounded-xl overflow-hidden">
                            ...
                            {skuRows.length > 0 && (
                                 <div className="bg-orange-50 p-3 flex flex-wrap gap-2 items-center text-xs border-b border-orange-100">
                                     ...
                                     <button onClick={() => {
                                         const p = Number((document.getElementById('bulk-price') as HTMLInputElement).value);
                                         ...
                                         handleBulkApply(p,s,k);
                                     }} className="bg-orange-600 text-white px-3 py-1 rounded font-bold hover:bg-orange-700 transition-colors">Áp dụng</button>
                                 </div>
                            )}
                            <table className="w-full text-sm text-left">
                                <thead className="text-xs text-gray-500 uppercase bg-gray-50 border-b border-gray-200">
                                    <tr>
                                        ...
                                        <th className="px-4 py-3 font-medium">Kho hàng</th>
                                        <th className="px-4 py-3 font-medium">SKU</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {skuRows.length === 0 ? (
                                        <tr><td colSpan={4} className="px-4 py-8 text-center text-gray-400 italic bg-white">Vui lòng thiết lập phân loại hàng trước</td></tr>
                                    ) : (
                                        skuRows.map((r, i) => (
              ...
              {/* Size Chart (Giữ nguyên) */}
              <div className="mb-8">
                  {/* ... (Code cũ của bạn cho Size chart) */}
                   <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex bg-gray-100 p-1 rounded-lg border border-gray-200">
                        <button 
                            onClick={() => setSizeChartMode('template')}
                            className={classNames("text-xs font-semibold px-3 py-1.5 rounded-md transition-all", sizeChartMode === 'template' ? "bg-white text-orange-600 shadow-sm" : "text-gray-500 hover:text-gray-700")}
                        >
                            Chọn Template
                        </button>
                        <button 
                             onClick={() => setSizeChartMode('image')}
                             className={classNames("text-xs font-semibold px-3 py-1.5 rounded-md transition-all", sizeChartMode === 'image' ? "bg-white text-orange-600 shadow-sm" : "text-gray-500 hover:text-gray-700")}
                        >
                            Tải ảnh lên
                        </button>
                    </div>
                ...
                {sizeChartMode === 'template' ? (
                     <div className="bg-gray-50/50 border border-dashed border-gray-300 rounded-xl p-6 flex flex-col items-center justify-center text-center transition-all">
                         ...
                         <button className="flex items-center gap-2 px-5 py-2.5 bg-white border border-orange-200 text-orange-600 text-sm font-bold rounded-lg hover:bg-orange-50 transition-all shadow-sm hover:shadow-md hover:-translate-y-0.5"><Plus size={16} strokeWidth={2.5} /> Tạo bảng quy đổi mới</button>
                     </div>
                ) : (
          ...
          {/* Section: Shipping (Giữ nguyên) */}
          <SectionCard title="Vận chuyển" id="shipping">
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        <FormLabel required subText="Trọng lượng sau đóng gói">Cân nặng (Gram)</FormLabel>
                        <InputField type="number" suffix="gr" value={weight} onChange={(e: any) => setWeight(Number(e.target.value))} />
                    </div>
                    <div>
          ...
          <SectionCard title="Cài đặt Mua kèm / Combo" id="cross-sell">
                <div className="mb-4">
                    <div className="bg-blue-50 text-blue-700 px-4 py-3 rounded-lg text-sm mb-4 flex items-start gap-2">
                        <Info size={16} className="mt-0.5 shrink-0"/>
                        <div>
                            <strong>Mẹo tăng doanh số:</strong> Chọn các sản phẩm thường được mua cùng nhau (Ví dụ: Giày + Tất, Điện thoại + Ốp lưng) để hiển thị trong mục "Thường được mua cùng" trên trang chi tiết.
                        </div>
                    </div>
                    ...
                    <CrossSellSelector 
                        ...
                        // Nếu đang mode Edit thì truyền ID sản phẩm hiện tại vào để tránh chọn chính nó
                        // currentProductId={productId} 
                    />
                </div>
            </SectionCard>

          {/* Section: Other Info (Giữ nguyên) */}
          <SectionCard title="Thông tin khác" id="others">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div><FormLabel>Tình trạng</FormLabel><SelectField value="Mới" placeholder="Chọn tình trạng" /></div>
                ...
                <div className="mx-auto w-[240px] border-4 border-gray-800 rounded-[2rem] overflow-hidden shadow-xl bg-white relative h-[480px]">
                    ...
                    <div className="h-full flex flex-col overflow-hidden bg-gray-50">
                        ...
                        <div className="p-3 flex-1 bg-white">
                             ...
                             {/* Hiển thị một vài thông số SEO trong preview để user thấy sướng */}
                             {extraAttributes.warranty_period && (
                                 <div className="mt-2 text-[10px] text-gray-500 bg-gray-100 px-1.5 py-0.5 rounded inline-block">BH: {extraAttributes.warranty_period}</div>
                             )}
                        </div>
