...
interface ProductCardProps {
  id?: string;
  image?: string; // [FIX] Cho phép optional
  title?: string;
  price?: string;
...
const ProductCard: React.FC<ProductCardProps> = ({
  ...
  const displayImage = image || "/assets/placeholder.png"; // [FIX] Ảnh fallback
  ...
  return (
    <div 
      ...
      `}
      onClick={() => { if (id) track(EventType.CLICK_PRODUCT, id); }}
    >
      ...
      <div className="p-2.5 flex flex-col flex-1 bg-white">
        <h3 className="text-xs text-gray-700 font-normal leading-relaxed line-clamp-2 min-h-[32px] mb-2 group-hover:text-orange-700 transition-colors duration-200" title={title}>
          {title}
        </h3>
        ...
        {flashSaleConfig ? (
          <div className="flex flex-col mt-auto gap-1">
            <div className="relative w-full h-3.5 bg-orange-100 rounded-[4px] overflow-hidden">
               <div className="absolute top-0 left-0 h-full bg-gradient-to-r from-orange-500 to-red-600 rounded-[4px] transition-all duration-500" style={{ width: `${percentSold}%` }}>
            ...
            <div className="flex items-baseline gap-1 mt-0.5">
              <span className="text-lg font-black text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-red-600">{price}</span>
              {originalPrice && <span className="text-[10px] text-gray-400 line-through decoration-gray-300 translate-y-[1px]">{originalPrice}</span>}
            </div>
          </div>
        ) : (
          <div className="flex flex-col gap-1 mt-auto">
            ...
            <div className="flex items-baseline gap-1 flex-wrap">
              ...
              {originalPrice && <span className="text-[10px] text-gray-400 line-through truncate decoration-gray-300">{originalPrice}</span>}
            </div>
